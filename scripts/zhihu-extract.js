 ...
 *
 * @module zhihu-extract
 */

import { extract as httpExtract } from './zhihu-http.js';
import { extractLog } from './zhihu-logger.js';

// ──────────────────────────────────────────
// 类型映射
// ──────────────────────────────────────────

/** @type {Record<string,string>} CLI 参数 → API 类型 */
const TYPE_MAP = {
  'hot-list': 'hotList',
  'hot_list': 'hotList',
  'hotList': 'hotList',
 ...
 *
 * @param {string} type - 数据类型（支持 CLI 格式和 API 格式）
 * @param {object} params - 参数
 * @param {boolean} [forceBrowser=false] - 强制浏览器通道
 * @returns {Promise<{ data: any, source: string, note?: string }>} 提取结果
 */
async function extract(type, params = {}, forceBrowser = false) {
  ...
  if (!forceBrowser) {
    try {
      ...
      extractLog.info(`HTTP 通道数据不可用，降级到浏览器`);
    } catch (err) {
      extractLog.warn('HTTP 通道失败，降级到浏览器通道', err);
    }
  }

  // 浏览器降级（待 Phase 2 实现完整逻辑）
  if (mappedType === 'hotList') {
    return { data: null, source: 'browser_pending', note: '浏览器通道数据提取需等待 Phase 2 开发完成' };
  }
  if (mappedType === 'search') {
    return { data: null, source: 'browser_pending', note: '浏览器通道搜索提取需等待 Phase 2 开发完成' };
  }
  if (mappedType === 'article') {
    return { data: null, source: 'browser_pending', note: '浏览器通道文章提取需等待 Phase 2 开发完成' };
  }
...
function main() {
  ...
  for (let i = 0; i < args.length; i++) {
    switch (args[i]) {
      ...
      case '--browser': options.forceBrowser = true; break;
      default:
  ...
  if (!options.type) {
    extractLog.error('用法: node scripts/zhihu-extract.js --type <type> [选项]');
    extractLog.error('类型: hot-list | search | article | user | question | answers');
    process.exit(1);
  }
  ...
  extract(options.type, options, options.forceBrowser)
    .then(result => {
      if (result.data && result.source === 'http') {
        ...
        extractLog.info(result.note);
      }
    })
    .catch(err => {
      extractLog.error('提取错误', err);
      process.exit(1);
    });
}
